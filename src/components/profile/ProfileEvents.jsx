...
export default function ProfileEvents() {
  ...
  const Row = ({ e }) => (
    <button onClick={() => navigate(`/events/${e.id}`)} className="w-full text-left flex items-center gap-3 bg-card border border-border shadow-soft rounded-2xl p-3">
      <div className="w-9 h-9 rounded-full bg-[#A1846B]/10 flex items-center justify-center"><CalendarHeart className="w-4 h-4 text-[#A1846B]" strokeWidth={1.5} /></div>
    ...
    <div className="mt-6">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-display font-semibold text-base flex items-center gap-1.5">
          <Users className="w-4 h-4 text-[#A1846B]" strokeWidth={1.5} /> My events
        </h3>
        <button onClick={() => navigate("/events")} className="text-xs text-[#A1846B]">View all</button>
      </div>
