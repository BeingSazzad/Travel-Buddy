            ...
            <div className="flex flex-col min-h-full">
              ...
              <div className="px-6 py-4 space-y-5 flex-1">
                ...
                {trips.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {trips.map((t, idx) => (
                        <div key={idx} className="rounded-2xl gradient-brand-soft border border-primary/25 p-3.5 shadow-sm">
                          <p className="text-sm font-display font-bold text-foreground">
                            {t.city}{t.country ? `, ${t.country}` : ""}
                          </p>
                          {t.dates && <p className="text-xs text-muted-foreground mt-0.5">{t.dates}</p>}
                          {t.travel_style && (
                            <span className="inline-block text-[10px] font-semibold text-primary capitalize px-2 py-0.5 rounded-full bg-primary/12 mt-1.5">
                              {t.travel_style}
                            </span>
                          )}
                        </div>
                ...
                {trips.length === 0 && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground py-2">
                    <Compass className="w-4 h-4 text-primary" /> No upcoming trips shared yet
                  </div>
                )}
              ...
              <div className="sticky bottom-0 bg-background/95 backdrop-blur-md border-t border-border/60 p-4">
                <Button
                  ...
                  className="w-full h-12 rounded-2xl bg-primary hover:bg-primary/90 text-white font-bold shadow-md flex items-center justify-center gap-2"
                >
