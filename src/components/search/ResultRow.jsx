...
export default function ResultRow({ item }) {
  ...
  return (
    <button
      type="button"
      className="w-full flex items-center gap-3 p-2.5 rounded-2xl hover:bg-card text-left active:scale-[0.99] transition"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1 mt-0.5 text-xs text-muted-foreground">
          ...
          <span className="truncate">{item.location}{item.country && item.country !== item.location ? `, ${item.country}` : ""}</span>
        </div>
